...
{
  return std::pow(p(0),4)+std::pow(p(1),4);
}
...
{
  //return p.square();
  return std::sin(10*p(0))*std::sin(5*p(1));
}
...
struct GridFunc
{
  ...
  {
    return std::asin(x) / asin(1);
  }
 
  Point<2> operator()(const Point<2> &in) const
  {
    return {trans(in(0)), trans(in(1))};
  }
};
...
{
  GridGenerator::hyper_cube_with_cylindrical_hole(triangulation, 0.5, 1);
  triangulation.refine_global(5);
  GridTools::transform(GridFunc(), triangulation);
